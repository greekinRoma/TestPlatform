...
def launch(datacache):
    ...
    trainer = Trainer(exp)
    trainer.train()
    test_exp = TestExp(args,datacache,data_dir=args['coco_data_dir'], save_dir=save_pth, use_tide=False,use_cuda=torch.cuda.is_available())
    test_exp.push_network(trainer.network)
